...
Usage
-----
    python eeg_prepost_stats_v2.py wide_feature_table.csv
"""
...
# ----------------------------------------------------------------------
# Heatmap: used for multi-measure families (currently only band_power,
# 5 ROI x 4 bands -- a genuine 2D grid). Only significant cells get a
# text label (stars + p_adj); non-significant cells are left blank.
# ----------------------------------------------------------------------
def make_family_heatmap(task_df, task_label, family_name, family_measures, out_path):
    family_df = task_df[task_df["Family"] == family_name]
    if family_df.empty:
        return False

    pivot_effect = family_df.pivot(index="ROI", columns="Measure", values="Effect_Size")
    pivot_effect = pivot_effect.reindex(index=ROIS, columns=family_measures)

    pivot_p = family_df.pivot(index="ROI", columns="Measure", values="p_adj_FDR")
    pivot_p = pivot_p.reindex(index=ROIS, columns=family_measures)

    n_cols = len(family_measures)
    fig_width = max(5.5, 1.8 * n_cols)
    ...
    ax.set_xticklabels(family_measures, rotation=45, ha="right", fontsize=9)
    ax.set_yticks(range(len(ROIS)))
    ax.set_yticklabels(ROIS, fontsize=9)

    for i in range(len(ROIS)):
        for j in range(n_cols):
            p = pivot_p.to_numpy()[i, j]
            if np.isnan(p) or p >= ALPHA:
                continue  # only label significant cells
            stars = sig_stars(p)
            ax.text(j, i, f"{stars}\np={p:.3f}", ha="center", va="center", fontsize=9,
                     fontweight="bold", color="white")

    n_tests = len(ROIS) * n_cols
    ax.set_title(f"Task {task_label} -- {family_name}\n(FDR family, n={n_tests} tests)"
                 f"\nColor = effect size, text shown only where significant", fontsize=10)
...
# ----------------------------------------------------------------------
# Dot + CI plot: used for single-measure families (theta_alpha_ratio,
# theta_beta_ratio, entropy -- only ROI varies, no second dimension, so
# a heatmap would just be one column). Shows mean difference + bootstrap
# 95% CI per ROI -- reuses the CI already computed in analyze_one_feature
# (a heatmap ignored this entirely). Same visual spirit as the
# estimation plots used for the cognitive-test analysis.
# ----------------------------------------------------------------------
def make_family_dotplot(task_df, task_label, family_name, family_measures, out_path):
    family_df = task_df[task_df["Family"] == family_name]
    if family_df.empty:
        return False

    family_df = family_df.set_index("ROI").reindex(ROIS)

    fig, ax = plt.subplots(figsize=(5.5, 4))
    y_pos = np.arange(len(ROIS))

    for i, roi in enumerate(ROIS):
        row = family_df.loc[roi]
        if pd.isna(row["Mean_Diff"]):
            continue
        mean_diff = row["Mean_Diff"]
        ci_lo = row["Boot_95CI_Low"]
        ci_hi = row["Boot_95CI_High"]
        p = row["p_adj_FDR"]
        is_sig = p < ALPHA

        color = "#C1272D" if mean_diff > 0 else "#1F77B4"
        color = color if is_sig else "lightgray"

        ax.errorbar(mean_diff, i, xerr=[[mean_diff - ci_lo], [ci_hi - mean_diff]],
                     fmt="o", color=color, ecolor=color, capsize=4, markersize=8,
                     markeredgecolor="black" if is_sig else "none", markeredgewidth=1)

        if is_sig:
            stars = sig_stars(p)
            ax.text(ci_hi, i, f"  {stars} p={p:.3f}", va="center", ha="left", fontsize=9, fontweight="bold")

    ax.axvline(0, color="gray", linestyle="--", linewidth=1)
    ax.set_yticks(y_pos)
    ax.set_yticklabels(ROIS, fontsize=10)
    ax.set_xlabel("Mean difference (Post - Pre), 95% bootstrap CI")
    ax.set_title(f"Task {task_label} -- {family_name}\n(FDR family, n={len(ROIS)} tests)", fontsize=10)
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    return True
...
def main(csv_path):
    ...
    for task, task_df in all_results.items():
        ...
        for family_name, family_measures in FEATURE_FAMILIES.items():
            plot_path = f"eeg_prepost_plot_task_{task}_{family_name}.png"
            if len(family_measures) > 1:
                made = make_family_heatmap(task_df, task, family_name, family_measures, plot_path)
            else:
                made = make_family_dotplot(task_df, task, family_name, family_measures, plot_path)
            if made:
