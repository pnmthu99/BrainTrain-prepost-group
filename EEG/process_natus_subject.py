...
# eye/muscle: well-established, high classification reliability (EEGLAB's
# own default; also the final choice in a published pipeline-comparison
# paper after testing 0.5-0.9). heart beat: added per literature review --
# ECG contamination has a very distinctive waveform, reliably classified,
# and NOT caught by any other step in this pipeline. line noise and
# channel noise deliberately excluded: line noise is already handled by
# the notch filter before ICA runs, and channel noise is largely handled
# by the explicit bad-channel detection/interpolation step -- auto-
# rejecting them again here would risk double-handling the same artifact
# through two different (and not perfectly consistent) mechanisms.
ICLABEL_AUTO_REJECT_CATEGORIES = {"eye blink", "muscle artifact", "heart beat"}
...
# ======================================================================
# Step 5 (was 2-4): resample, montage + interpolate, ONE wide-band filter
# (1-100Hz, matches ICLabel's expected range) -- done AFTER concatenation.
# The narrower 0.5-45Hz analysis filter is applied LATER, AFTER ICA
# cleaning, directly on the epochs (see run_ica_iclabel + main()). This
# avoids maintaining two parallel raw/epoch copies -- much simpler than
# the earlier branching approach, same result.
# ======================================================================
def preprocess(raw):
...
def build_epochs(raw, stage_label=""):
    events_list = []
...
# ======================================================================
# Step 6: ICA + ICLabel -- fit AND apply directly on the epochs (matches
# the proven prior BrainTrain script's structure: ica.fit(epochs) ->
# ica.apply(epochs, exclude=...)). Edge-filtering concerns that would
# favor continuous-data processing don't meaningfully apply here since
# these are 60s epochs, not short ~1-3s ERP epochs.
# ======================================================================
...
def main():
    ...
    # range). For 60s epochs, FIR filter edge-effects at each epoch's two
    # boundaries are negligible relative to the epoch length, so there's
    # no real benefit to filtering on continuous data first (that
    # precaution matters much more for short ~1-3s ERP epochs, not here).
    # This matches the proven prior BrainTrain script's structure:
    # epoch -> ICA -> apply -> filter, all on the SAME epochs object.
