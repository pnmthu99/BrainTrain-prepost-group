# -*- coding: utf-8 -*-
"""Aggregate and analyze one EEG feature family with longitudinal LMMs.
...
ROI feature columns. Select a family to aggregate directly from these files and
fit one LMM per task x ROI x measure using planned contrasts. Relative power
also receives a composition check: the four bands should sum to approximately
one.

Requires eeg_longitudinal_lmm_stats.py in the same folder.
Usage:
    python eeg_feature_family_lmm.py features_folder [family]

Families: relative_power, theta_alpha_ratio, theta_beta_ratio, entropy.
If family is omitted, relative_power is used for backwards compatibility.
"""
...
from eeg_longitudinal_lmm_stats import (
    ...
    make_lmm_dotplot,
    make_lmm_heatmap,
    make_lmm_integrated_trajectory,
    make_lmm_trajectory,
    rounded_for_export,
)
...
FAMILY_CONFIG = {
    "relative_power": {
        "measures": [f"{band}_relpower" for band in BANDS],
        "output_folder": "results_relative_power_lmm",
        "use_relative_power_qc": True,
    },
    "theta_alpha_ratio": {
        "measures": ["theta_alpha_ratio"],
        "output_folder": "results_theta_alpha_ratio_lmm",
        "use_relative_power_qc": False,
    },
    "theta_beta_ratio": {
        "measures": ["theta_beta_ratio"],
        "output_folder": "results_theta_beta_ratio_lmm",
        "use_relative_power_qc": False,
    },
    "entropy": {
        "measures": ["entropy"],
        "output_folder": "results_entropy_lmm",
        "use_relative_power_qc": False,
    },
}
...
def load_source_features(features_dir, measures):
        ...
        for _, row in source.iterrows():
            ...
            for roi in ROIS:
                for measure in measures:
                    source_column = f"{roi}_{measure}"
                    output_column = f"{task}_{roi}_{measure}_{timepoint}"
...
def build_family_long(df, subject_ids, family, measures):
    frames = []
    for task in TASKS:
        for roi in ROIS:
            for measure in measures:
                ...
                frames.append(long)
    if not frames:
        raise ValueError("No requested feature-family columns were found.")
    return pd.concat(frames, ignore_index=True)[
        ["subject_id", "Task", "ROI", "Family", "Measure", "timepoint", "value"]
    ]
...
def build_relative_power_qc(long):
    relative_long = long.copy()
    relative_long["Band"] = relative_long["Measure"].str.replace("_relpower", "", regex=False)
    return build_qc(relative_long)


def build_family_qc(long):
    return long.groupby(["Task", "ROI", "Family", "Measure", "timepoint"], observed=False)["value"].agg(
        N_total="size", N_available="count", Mean="mean", SD="std", Median="median"
    ).reset_index().assign(N_missing=lambda table: table["N_total"] - table["N_available"])


def run_models(df, subject_ids, family, measures):
    ...
    for task in TASKS:
        ...
        for roi in ROIS:
            for measure in measures:
                ...
                metadata = {"Task": task, "Family": family, "ROI": roi, "Measure": measure}
...
def main(features_dir, family):
    config = FAMILY_CONFIG[family]
    measures = config["measures"]
    df, source_manifest = load_source_features(features_dir, measures)
    ...
    long = build_family_long(df, subject_ids, family, measures)
    qc = build_relative_power_qc(long) if config["use_relative_power_qc"] else build_family_qc(long)
    omnibus_by_task, contrasts_by_task, trajectories, failures = run_models(df, subject_ids, family, measures)
    if not omnibus_by_task:
        raise ValueError(f"No {family} LMMs could be fitted.")
    ...
    df.to_csv(output_dir / f"{family}_wide_from_source.csv", index=False)
    source_manifest.to_csv(output_dir / f"{family}_source_manifest.csv", index=False)
    long.to_csv(output_dir / f"{family}_long.csv", index=False)
    rounded_for_export(qc).to_csv(output_dir / f"{family}_qc.csv", index=False)
    rounded_for_export(omnibus_summary).to_csv(output_dir / f"{family}_lmm_omnibus_summary.csv", index=False)
    rounded_for_export(contrasts_summary).to_csv(output_dir / f"{family}_lmm_contrasts_summary.csv", index=False)
    failures.to_csv(output_dir / f"{family}_lmm_failures.csv", index=False)

    q_by_feature = omnibus_summary.set_index(["Task", "ROI", "Measure"])["p_adj_FDR"]
    integrated_dir = output_dir / "integrated_figures"
    integrated_dir.mkdir(exist_ok=True)
    for task, task_omnibus in omnibus_by_task.items():
        rounded_for_export(task_omnibus).to_csv(output_dir / f"{family}_lmm_omnibus_task_{task}.csv", index=False)
        task_contrasts = contrasts_by_task[task]
        rounded_for_export(task_contrasts).to_csv(output_dir / f"{family}_lmm_contrasts_task_{task}.csv", index=False)
        for contrast_name in ["post_minus_pre", "3m_minus_pre", "3m_minus_post"]:
            plot_path = output_dir / f"{family}_task_{task}_{contrast_name}.png"
            if len(measures) > 1:
                make_lmm_heatmap(task_contrasts, task, family, measures, contrast_name, plot_path)
            else:
                make_lmm_dotplot(task_contrasts, task, family, contrast_name, plot_path)
    ...
    for metadata, feature_long, model_means in trajectories:
        ...
        make_lmm_trajectory(
            ...
            output_dir / f"{family}_trajectory_task_{key[0]}_{key[1]}_{metadata['Measure']}.png"
        )
        feature_contrasts = contrasts_by_task[key[0]]
        feature_contrasts = feature_contrasts[
            (feature_contrasts["ROI"] == key[1]) & (feature_contrasts["Measure"] == key[2])
        ]
        make_lmm_integrated_trajectory(
            metadata, feature_long, model_means, q_by_feature.loc[key], feature_contrasts,
            integrated_dir / f"{family}_integrated_task_{key[0]}_{key[1]}_{metadata['Measure']}.png"
        )


if __name__ == "__main__":
    if len(sys.argv) not in (2, 3):
        print("Usage: python eeg_feature_family_lmm.py features_folder [family]")
        sys.exit(1)
    selected_family = sys.argv[2] if len(sys.argv) == 3 else "relative_power"
    if selected_family not in FAMILY_CONFIG:
        print(f"Unknown family: {selected_family}. Choose one of: {', '.join(FAMILY_CONFIG)}")
        sys.exit(1)
    main(sys.argv[1], selected_family)
